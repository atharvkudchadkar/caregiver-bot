...
The desktop viewer remains the simulation display. See README.md for setup.
"""
...
class ArmFollower:
    ...
    def follow(self, side, controller, head):
        ...
        if side not in self.origin:
            self.origin[side] = (
                Rotation.from_quat(controller["orientation"]).as_matrix(),
                base_rot.T @ d.site_xmat[site_id].reshape(3, 3),
            )
...
class QuestTeleop:
    def __init__(self, model, data, receiver):
        ...
        self.arms.hold_current()
        self.follower = ArmFollower(model, data, self.arms)
    ...
    def tick(self):
        ...
        if d.time - self.last_control >= CONTROL_PERIOD_S:
            ...
            active = packet is not None and packet["active"] and age < TIMEOUT_S
            if active:
...
def main():
    ...
    try:
        teleop = QuestTeleop(model, data, receiver)
